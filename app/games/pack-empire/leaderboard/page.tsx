...
interface LeaderboardEntry {
  name: string; score: number; tier: string; icon: string;
...
/* ─── Mini filled card for the lineup view ──────────────────────── */
...
export default function LeaderboardPage() {
  ...
  supabase
    ...
    // load player images from CSV — same logic as offense page
  ...
  return (
    <>
      ...
      <nav className="lb-nav">
        ...
        <Link href="/games/pack-empire" className="lb-hub">← Pack Empire</Link>
      </nav>
      ...
      <div className="lb-root">
        <div className="lb-hero">
          ...
          <div className="lb-hero-sub">{entries.length} draft{entries.length !== 1 ? 's' : ''} submitted · click any row to view the full lineup</div>
        </div>
          ...
          <div className="lb-table">
            ...
            {entries.map((entry, i) => (
              <div
                ...
                className={`lb-row${i < 3 ? ' top3' : ''}${selected?.timestamp === entry.timestamp ? ' active' : ''}`}
                onClick={() => setSelected(selected?.timestamp === entry.timestamp ? null : entry)}
              >
                ...
                <div className="lb-td action">
                  <span className="lb-chevron">{selected?.timestamp === entry.timestamp ? '▲' : '▼'}</span>
                </div>

                {/* Inline lineup expand */}
                {selected?.timestamp === entry.timestamp && entry.lineup && (
...
.lb-back{color:#2a4060;text-decoration:none;font-family:'Barlow Condensed',sans-serif;font-size:.82rem;
  letter-spacing:.08em;transition:.15s;justify-self:start}
.lb-back:hover{color:#d4e8f8}
.lb-hub{color:#2a4060;text-decoration:none;font-family:'Barlow Condensed',sans-serif;font-size:.82rem;
  letter-spacing:.08em;transition:.15s;justify-self:end}
.lb-hub:hover{color:#d4e8f8}
.lb-nav-title{display:flex;align-items:center;gap:.55rem;font-family:'Orbitron',sans-serif;font-weight:700;
  font-size:.9rem;letter-spacing:.22em;color:#ffd700;justify-self:center;text-shadow:0 0 20px rgba(255,215,0,.5)}
.lb-pip{width:8px;height:8px;border-radius:50%;background:#ffd700;flex-shrink:0;
  box-shadow:0 0 10px #ffd700;animation:pip-pulse 2s ease-in-out infinite}
...
/* hero */
.lb-hero{text-align:center;padding:2rem 1rem 1.5rem;
  background:radial-gradient(ellipse at 50% 0%,rgba(255,215,0,.07),transparent 60%)}
.lb-hero-sup{font-size:.6rem;letter-spacing:.42em;color:#3a6080;margin-bottom:.3rem}
.lb-hero-title{font-family:'Orbitron',sans-serif;font-weight:900;font-size:clamp(2rem,6vw,3.5rem);
...
.lb-empty-title{font-family:'Orbitron',sans-serif;font-weight:700;font-size:1.1rem;letter-spacing:.1em;
  color:#2a4060;margin-bottom:.4rem}
.lb-empty-sub{font-size:.85rem;color:#1a3050;margin-bottom:1.5rem;letter-spacing:.06em}
.lb-empty-cta{display:inline-block;background:linear-gradient(135deg,#a07020,#ffd700);color:#050a18;
  font-family:'Orbitron',sans-serif;font-weight:700;font-size:.8rem;letter-spacing:.16em;
  padding:.75rem 1.6rem;border-radius:8px;text-decoration:none;transition:.2s}
...
.lb-thead{display:grid;grid-template-columns:52px 1fr 200px 110px 36px;
  padding:.55rem .9rem;background:rgba(4,8,16,.95);border-bottom:1px solid #0d1835}
.lb-th{font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:.6rem;
  letter-spacing:.2em;color:#2a4060}
...
.lb-row{display:grid;grid-template-columns:52px 1fr 200px 110px 36px;
  padding:.65rem .9rem;border-bottom:1px solid #0a1228;cursor:pointer;
  transition:background .15s;position:relative;align-items:center}
...
.lb-entry-name{font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:.92rem;
  color:#d4e8f8;letter-spacing:.04em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.lb-entry-tier{font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:.72rem;
  letter-spacing:.08em;white-space:nowrap}
.lb-entry-score{font-family:'Orbitron',sans-serif;font-weight:900;font-size:.85rem;
  color:#d4e8f8;margin-left:auto}
...
/* lineup panel — spans full row width */
.lb-lineup-panel{
  grid-column:1/-1;
  background:rgba(4,8,16,.95);
  border-top:1px solid #0d1835;
  padding:1.2rem;
...
.lb-lineup-name{font-family:'Orbitron',sans-serif;font-weight:700;font-size:1rem;
  letter-spacing:.1em;color:#d4e8f8}
...
.lb-pos-label{font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:.58rem;
  letter-spacing:.22em;color:#2a4060;margin-bottom:.4rem}
.lb-card-row{display:flex;gap:8px;flex-wrap:wrap}

/* roster list */
.lb-roster{display:flex;flex-direction:column;gap:4px;
  background:rgba(2,6,14,.8);border:1px solid #0d1835;border-radius:8px;padding:.7rem .8rem}
.lb-roster-row{display:flex;align-items:center;gap:.6rem;padding:.2rem 0;
  border-bottom:1px solid #080e20;font-family:'Barlow Condensed',sans-serif}
.lb-roster-row:last-child{border-bottom:none}
.lb-roster-pos{font-weight:800;font-size:.58rem;letter-spacing:.1em;color:#fff;
  padding:1px 5px;border-radius:2px;min-width:26px;text-align:center}
