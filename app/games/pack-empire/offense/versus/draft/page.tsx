...
export default function VersusDraft() {
  ...
  useEffect(() => {
    ...
    async function checkIdentity() {
      ...
      const host = !!(thisChallenge && thisChallenge.creatorId === user?.id);
      setIsHost(host);
      ...
      if (savedGuest) { setGuestName(savedGuest); setNeedsIdentity(false); }
...
{needsIdentity && (
  <div style={{ maxWidth: 480, margin: '60px auto', padding: '0 1.5rem', textAlign: 'center' }}>
    ...
    {/* Sign in option */}
    <Link
      href={`/login?redirect=/games/pack-empire/offense/versus/draft?challenge=${challengeId}`}
      style={{
...
.pe-nav{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;padding:.7rem 1.2rem;border-bottom:2px solid #0d1835;position:sticky;top:0;background:#050a18;z-index:30}
...
@media(max-width:700px){.pe-pack-img,.pe-fsl{width:82px;height:128px}.pe-card{width:calc(19vw - 4px);min-width:110px}.pe-cards-row{gap:.4rem}.pe-skill-spread{width:calc(100% + 100px);margin:0 -20px}.pe-form-row{gap:6px}.pe-skill-side{gap:6px}.pe-pack-wrap,.pe-fsl{width:82px;height:128px}}
